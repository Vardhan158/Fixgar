...
function SectionCard({ title, icon, children, accent = false }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
      <div
...
/* ─── DRAGGABLE MAP PIN ──────────────────────────────────────── */
function DraggableMap({ coords, onPinMoved }) {
  const frameRef = useRef(null);
  const pinRef = useRef(null);
  const isDragging = useRef(false);
  const startPos = useRef({ x: 0, y: 0 });
  const pinOffset = useRef({ x: 50, y: 50 }); // percent
  ...
  const onStart = (e) => {
    ...
    if (pinRef.current) pinRef.current.style.transform = "translate(-50%, -110%) scale(1.15)";
  };
  ...
  const onMove = useCallback((e) => {
    ...
    pinOffset.current = {
      ...
      y: Math.max(10, Math.min(85, 50 + dy)),
    };
    if (pinRef.current) {
      pinRef.current.style.left = pinOffset.current.x + "%";
      pinRef.current.style.top = pinOffset.current.y + "%";
    }
  }, []);
  ...
  const onEnd = useCallback(() => {
    ...
    if (pinRef.current) pinRef.current.style.transform = "translate(-50%, -100%) scale(1)";
    onPinMoved(pinOffset.current);
  }, [onPinMoved]);
  ...
  const mapSrc = coords
    ? `https://www.openstreetmap.org/export/embed.html?bbox=${coords.lon - 0.003},${coords.lat - 0.003},${coords.lon + 0.003},${coords.lat + 0.003}&layer=mapnik`
    : null;
  ...
  return (
    <div
      ...
      style={{ height: 200, cursor: "grab", userSelect: "none" }}
    >
      {mapSrc ? (
        <iframe
          src={mapSrc}
        ...
        /* Placeholder SVG map when no coords yet */
        <svg viewBox="0 0 400 200" width="100%" height="200" style={{ display: "block", background: "#dde8f5" }}>
          <defs>
            <pattern id="mapgrid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M40 0L0 0 0 40" fill="none" stroke="#c5d8f0" strokeWidth="0.5" />
            </pattern>
          </defs>
          <rect width="400" height="200" fill="url(#mapgrid)" opacity="0.5" />
          <rect x="10" y="10" width="75" height="52" rx="4" fill="#c8d8ed" />
          <rect x="95" y="10" width="95" height="52" rx="4" fill="#c8d8ed" />
          <rect x="200" y="10" width="85" height="52" rx="4" fill="#c8d8ed" />
          <rect x="295" y="10" width="95" height="52" rx="4" fill="#c8d8ed" />
          <rect x="10" y="78" width="58" height="62" rx="4" fill="#c8d8ed" />
          <rect x="78" y="78" width="115" height="62" rx="4" fill="#c8d8ed" />
          <rect x="203" y="78" width="65" height="62" rx="4" fill="#c8d8ed" />
          <rect x="278" y="78" width="112" height="62" rx="4" fill="#c8d8ed" />
          <rect x="10" y="153" width="105" height="38" rx="4" fill="#c8d8ed" />
          <rect x="125" y="153" width="85" height="38" rx="4" fill="#c8d8ed" />
          <rect x="220" y="153" width="95" height="38" rx="4" fill="#c8d8ed" />
          <rect x="325" y="153" width="65" height="38" rx="4" fill="#c8d8ed" />
          <rect x="0" y="68" width="400" height="8" fill="white" opacity="0.8" />
          <rect x="0" y="143" width="400" height="8" fill="white" opacity="0.8" />
          <rect x="73" y="0" width="6" height="200" fill="white" opacity="0.8" />
          <rect x="197" y="0" width="6" height="200" fill="white" opacity="0.8" />
          <rect x="274" y="0" width="6" height="200" fill="white" opacity="0.8" />
          <text x="200" y="105" textAnchor="middle" fontSize="12" fill="#94a3b8" fontFamily="sans-serif">Tap "Use my location" to load map</text>
        </svg>
      )}
      ...
      >
        <svg width="32" height="44" viewBox="0 0 32 44" style={{ filter: "drop-shadow(0 4px 8px rgba(0,0,0,0.3))", display: "block" }}>
          <path d="M16 0C7.16 0 0 7.16 0 16c0 11.55 16 28 16 28s16-16.45 16-28C32 7.16 24.84 0 16 0z" fill="#2563eb" />
          <circle cx="16" cy="16" r="6.5" fill="white" />
          <circle cx="16" cy="16" r="3" fill="#2563eb" />
        </svg>
        {/* Shadow dot */}
        <div style={{ width: 10, height: 4, background: "rgba(0,0,0,0.18)", borderRadius: "50%", margin: "0 auto" }} />
      </div>
      ...
      <div
        style={{
          ...
          padding: "3px 12px", fontSize: 11, color: "#6b7280", whiteSpace: "nowrap",
...
function AccuracyBar({ accuracy }) {
  ...
  const pct = Math.max(5, Math.min(100, 100 - (accuracy / 200) * 100));
  const { label, color } =
    accuracy <= 20 ? { label: "✓ Precise", color: "#10b981" }
    : accuracy <= 50 ? { label: "Good", color: "#22c55e" }
    : accuracy <= 100 ? { label: "Fair", color: "#f59e0b" }
  ...
  return (
    <div className="px-1 mt-1">
      <div className="flex justify-between text-xs mb-1" style={{ fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <span className="font-semibold" style={{ color }}>{label} · ±{accuracy}m</span>
      </div>
...
function LiveMap({ providerPos }) {
  return (
    <div style={{ position: "relative", width: "100%", height: 220, overflow: "hidden", background: "#e8f0fe" }}>
      <svg viewBox="0 0 400 220" width="100%" height="220" style={{ position: "absolute", top: 0, left: 0 }}>
        ...
        <rect width="400" height="220" fill="url(#grid2)" opacity="0.3" />
        ...
        <polyline points="75,195 75,140 75,68 185,68 185,40 272,40" stroke="#3b82f6" strokeWidth="4" fill="none" strokeDasharray="8 4" strokeLinecap="round" opacity="0.8" />
...
function TrackingScreen({ provider, address, date, timeSlot, total, onBack }) {
  const waypoints = [
    { x: 75, y: 195 }, { x: 75, y: 140 }, { x: 75, y: 68 },
    { x: 185, y: 68 }, { x: 185, y: 40 }, { x: 268, y: 40 },
  ];
  const [posIdx, setPosIdx] = useState(0);
  const [providerPos, setProviderPos] = useState(waypoints[0]);
  const animRef = useRef(null);
  const startRef = useRef(null);
  const SEGMENT_DURATION = 2200;
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      ...
      setProviderPos({ x: from.x + (to.x - from.x) * t, y: from.y + (to.y - from.y) * t });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <style>{`
        ...
        .pulse-ring { position:absolute;inset:0;border-radius:50%;border:2px solid #3b82f6;animation:pulseRing 1.6s ease-out infinite; }
      `}</style>
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 flex items-center gap-3 px-4 py-3" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <button onClick={onBack} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors">
          <ArrowLeft />
        </button>
        <div className="flex-1">
          <div className="font-bold text-gray-900 text-sm" style={{ fontFamily: "'Sora',sans-serif" }}>Live Tracking</div>
          <div className="text-xs text-gray-400" style={{ fontFamily: "'DM Sans',sans-serif" }}>{provider.name} · on the way</div>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: "#ecfdf5", color: "#065f46", border: "1px solid #a7f3d0" }}>
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block" />
          ETA {provider.eta}
        </div>
      </div>
      <div className="max-w-md mx-auto">
        <div className="track-slide"><LiveMap providerPos={providerPos} /></div>
        <div className="px-4 py-4 space-y-4">
          <div className="track-slide bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ animationDelay: "60ms", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
            <div className="p-4 flex items-center gap-4">
              <div className="relative flex-shrink-0" style={{ width: 52, height: 52 }}>
                <div className="pulse-ring" style={{ top: -3, left: -3, right: -3, bottom: -3, borderRadius: "50%" }} />
                <div className="w-full h-full rounded-full flex items-center justify-center text-white font-black text-base" style={{ background: "linear-gradient(135deg,#3b82f6,#1d4ed8)" }}>
                  {provider.avatar}
                </div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-gray-900 text-sm truncate" style={{ fontFamily: "'Sora',sans-serif" }}>{provider.name}</div>
              ...
              <a href={`tel:${provider.phone}`} className="flex flex-col items-center justify-center w-12 h-12 rounded-xl border border-emerald-100 bg-emerald-50 hover:bg-emerald-100 transition-colors flex-shrink-0">
            ...
            <div className="px-4 pb-4">
              <div className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs" style={{ background: "#f0fdf4", border: "1px solid #d1fae5" }}>
                <div className="text-emerald-500"><PhoneIcon /></div>
          ...
          <div className="track-slide bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ animationDelay: "120ms", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
              ...
              {STATUS_STEPS.map((s, i) => (
                <div key={s.label} className="flex gap-3 items-stretch">
                  <div className="flex flex-col items-center" style={{ minWidth: 20 }}>
                    <div style={{ width: 12, height: 12, borderRadius: "50%", flexShrink: 0, marginTop: 3, background: s.done ? "#10b981" : s.active ? "#3b82f6" : "#d1d5db", boxShadow: s.active ? "0 0 0 4px rgba(59,130,246,0.2)" : "none", transition: "all 0.3s" }} />
                    {i < STATUS_STEPS.length - 1 && (
                      <div style={{ width: 2, flex: 1, minHeight: 20, margin: "3px 0", background: s.done ? "linear-gradient(180deg,#10b981,#3b82f6)" : "#e5e7eb", borderRadius: 2, transition: "background 0.5s" }} />
                    )}
                  </div>
                  <div className="pb-4 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold" style={{ fontFamily: "'Sora',sans-serif", color: s.done ? "#059669" : s.active ? "#1d4ed8" : "#9ca3af" }}>{s.label}</span>
                      {s.active && <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#eff6ff", color: "#1d4ed8" }}>Live</span>}
                      {s.done && <span className="text-xs text-emerald-500">✓</span>}
                    </div>
                    <div className="text-xs mt-0.5" style={{ fontFamily: "'DM Sans',sans-serif", color: "#9ca3af" }}>{s.sub}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="track-slide bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ animationDelay: "180ms", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
            ...
            <div className="p-5 space-y-2.5 text-sm">
              {[["📍 Address", address], ["📅 Date", date], ["🕐 Slot", slotLabel ? `${slotLabel.label} · ${slotLabel.sub}` : "—"], ["💳 Total", `₹${total}`]].map(([label, val]) => (
          ...
          <div className="track-slide rounded-xl px-4 py-3 flex gap-3 items-start" style={{ animationDelay: "240ms", background: "#fffbeb", border: "1px solid #fde68a" }}>
            <span className="text-base mt-0.5">💡</span>
            <p className="text-xs text-amber-700" style={{ fontFamily: "'DM Sans',sans-serif" }}>Pay only after the job is done. Never pay in advance. If you feel unsafe, call our support line.</p>
          </div>

          <div className="track-slide pb-6" style={{ animationDelay: "300ms" }}>
            <button onClick={onBack} className="w-full border border-gray-200 text-gray-600 hover:border-gray-300 font-semibold text-sm rounded-xl py-3 transition-all duration-200" style={{ fontFamily: "'Sora',sans-serif" }}>
              ← Back to Booking
            </button>
          </div>
        </div>
...
export default function Booking({ initialService = DEFAULT_SERVICE }) {
  ...
  const [locCoords, setLocCoords] = useState(null); // { lat, lon }
  const [locStatus, setLocStatus] = useState(""); // descriptive status string
  ...
  useEffect(() => {
    if (status === "searching") {
      setSearchProgress(0);
      progressRef.current = setInterval(() => setSearchProgress(p => Math.min(p + 2, 95)), 80);
      timerRef.current = setTimeout(() => {
        clearInterval(progressRef.current);
        setSearchProgress(100);
        setTimeout(() => { setProvider(DUMMY_PROVIDER); setStatus("assigned"); setStep(3); }, 400);
      }, 4200);
    }
  ...
  const geocodeBigDataCloud = async (lat, lon) => {
    ...
    const parts = [d.locality || "", d.city || d.principalSubdivision || "", d.countryName || "", d.postcode || ""].filter(Boolean);
    return parts.join(", ");
  };
  ...
  const reverseGeocode = async (lat, lon) => {
    try { return await geocodeNominatim(lat, lon); }
      ...
      catch { return `${lat.toFixed(5)}°N, ${lon.toFixed(5)}°E`; }
    }
  };
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        setLocCoords({ lat, lon });
        setLocAccuracy(Math.round(position.coords.accuracy));
  ...
  /* ── Pin dragged on map ── */
  const handlePinMoved = useCallback((_pinPercent) => {
    // When user drags the pin, clear GPS lock and prompt them to confirm
    setLocStatus("Pin moved · address may differ from GPS");
    setAddress(prev => prev ? `📍 ${prev}` : prev);
  }, []);
  ...
  const handleReset = () => {
    ...
    setErrors({}); setStatus("idle"); setProvider(null); setStep(0);
    setSearchProgress(0); setLocCoords(null); setLocAccuracy(null);
    setLocStatus(""); setShowMap(false); setLocError("");
    stopLiveLocation();
  };
  ...
  if (status === "tracking" && provider) {
    return <TrackingScreen provider={provider} address={address} date={date} timeSlot={timeSlot} total={total} onBack={() => setStatus("assigned")} />;
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6 py-12">
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="bg-white rounded-3xl p-10 flex flex-col items-center" style={{ boxShadow: "0 8px 40px rgba(59,130,246,0.12)" }}>
            <div className="relative flex items-center justify-center mb-6">
              ...
              <div className="relative w-20 h-20 rounded-full flex items-center justify-center text-3xl" style={{ background: "linear-gradient(135deg,#3b82f6,#2563eb)", boxShadow: "0 8px 24px rgba(59,130,246,0.4)" }}>🔍</div>
            </div>
            <h2 className="text-xl font-extrabold text-gray-900 mb-2" style={{ fontFamily: "'Sora',sans-serif" }}>Finding your {service.name}</h2>
            <p className="text-gray-400 text-sm mb-6" style={{ fontFamily: "'DM Sans',sans-serif" }}>Connecting you with verified professionals nearby…</p>
            <div className="w-full bg-gray-100 rounded-full h-2 mb-2 overflow-hidden">
              <div className="h-2 rounded-full transition-all duration-200" style={{ width: `${searchProgress}%`, background: "linear-gradient(90deg,#3b82f6,#38bdf8)" }} />
            </div>
            ...
            <button onClick={handleCancel} className="flex items-center gap-2 text-sm text-gray-500 hover:text-red-500 border border-gray-200 hover:border-red-200 rounded-xl px-5 py-2.5 transition-all duration-200" style={{ fontFamily: "'Sora',sans-serif" }}>
              <XIcon /> Cancel Search
            </button>
          </div>
        </div>
  ...
  if (status === "assigned" && provider) {
    return (
      <div className="min-h-screen bg-gray-50 px-6 py-12">
        ...
        <div className="max-w-md mx-auto">
          <StepIndicator current={step} />
          <div className="rounded-2xl p-5 mb-4 text-center" style={{ background: "linear-gradient(135deg,#ecfdf5,#d1fae5)", border: "1px solid #a7f3d0" }}>
            <div className="flex justify-center mb-2"><CheckCircle /></div>
            <h2 className="font-extrabold text-emerald-800 text-lg" style={{ fontFamily: "'Sora',sans-serif" }}>Provider Assigned!</h2>
            <p className="text-emerald-600 text-sm mt-1" style={{ fontFamily: "'DM Sans',sans-serif" }}>Your {service.name} is on the way 🎉</p>
          </div>
          <div className="bg-white rounded-2xl overflow-hidden mb-4" style={{ boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
            ...
            <div className="p-5">
              <div className="flex items-center gap-4 mb-5">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-black text-lg flex-shrink-0" style={{ background: "linear-gradient(135deg,#3b82f6,#1d4ed8)", boxShadow: "0 4px 14px rgba(59,130,246,0.4)" }}>
                  {provider.avatar}
                </div>
                <div className="flex-1">
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                ...
                <a href={`tel:${provider.phone}`} className="flex flex-col items-center bg-emerald-50 rounded-xl py-3 hover:bg-emerald-100 transition-colors cursor-pointer">
          ...
          <div className="flex gap-3">
            <button onClick={handleReset} className="flex-1 border border-gray-200 text-gray-600 hover:border-gray-300 font-semibold text-sm rounded-xl py-3 transition-all duration-200" style={{ fontFamily: "'Sora',sans-serif" }}>
              Book Another
            </button>
            <button onClick={() => setStatus("tracking")} className="flex-1 text-white font-bold text-sm rounded-xl py-3 transition-all duration-200 hover:opacity-90" style={{ background: "linear-gradient(135deg,#3b82f6,#2563eb)", fontFamily: "'Sora',sans-serif", boxShadow: "0 4px 16px rgba(59,130,246,0.35)" }}>
              Track Live 📍
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-10">
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-6 slide-in">
          <h1 className="text-2xl font-extrabold text-gray-900" style={{ fontFamily: "'Sora',sans-serif", letterSpacing: "-0.02em" }}>Book a Service</h1>
          <p className="text-gray-400 text-sm mt-1" style={{ fontFamily: "'DM Sans',sans-serif" }}>Fill in the details and we'll find you the best pro nearby.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="slide-in" style={{ animationDelay: "0ms" }}>
            <SectionCard title="Selected Service" icon="🛠" accent>
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0" style={{ background: "linear-gradient(135deg,#eff6ff,#dbeafe)" }}>
                  {service.emoji}
                </div>
                <div className="flex-1">
            ...
            <SectionCard title="Service Address" icon="📍">
              <div className="space-y-3">
                ...
                <div>
                  <input
                    ...
                    onChange={e => {
                      ...
                      if (e.target.value) setErrors(prev => ({ ...prev, address: undefined }));
                      setStep(s => Math.max(s, 1));
                    }}
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 text-sm font-semibold border rounded-xl px-4 py-2.5 transition-all duration-200 ${
                    locLoading
                      ? "text-blue-600 border-blue-200 bg-blue-50"
                      : "text-blue-600 border-blue-100 hover:border-blue-300 bg-blue-50 hover:bg-blue-100"
                  }`}
                  ...
                  {locLoading ? (
                    <>
                      ...
                      <span className="flex-1 text-left">
                        {locAccuracy !== null
                          ? `Refining location… ±${locAccuracy}m`
                          : "Acquiring GPS signal…"}
                      </span>
                      <span
                        className="text-xs text-red-400 font-medium border border-red-200 rounded-full px-2.5 py-0.5 hover:bg-red-50 transition-colors"
                        onClick={e => { e.stopPropagation(); stopLiveLocation(); setLocStatus("Stopped"); }}
                      >
                        Stop
                      </span>
                    </>
                    ...
                    <>
                      ...
                      <span className="ml-auto text-xs text-blue-400 font-normal">Like Zomato / Swiggy</span>
                    </>
                  )}
                </button>

                {/* Accuracy bar */}
                {locLoading && <AccuracyBar accuracy={locAccuracy} />}

                {/* Status text */}
                {locStatus && !locError && (
                  <div className="flex items-center gap-2 text-xs px-1" style={{ fontFamily: "'DM Sans',sans-serif" }}>
                    {locLoading
                    ...
                    <span className="text-gray-500">{locStatus}</span>
                  </div>
                )}
                ...
                {/* Draggable map — shown once location is detected */}
                {showMap && (
                  <div>
                    <div className="flex items-center justify-between mb-1.5 px-0.5">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide" style={{ fontFamily: "'DM Sans',sans-serif" }}>
                        Drag pin to adjust
                      </span>
                      <button
                        className="text-xs text-blue-500 hover:text-blue-700 font-medium transition-colors"
                        style={{ fontFamily: "'DM Sans',sans-serif" }}
                        onClick={() => { setShowMap(false); setLocCoords(null); setLocAccuracy(null); setLocStatus(""); }}
                      >
                        Hide map
                      </button>
                    </div>
                    <DraggableMap coords={locCoords} onPinMoved={handlePinMoved} />
                    <p className="text-xs text-gray-400 mt-1.5 px-0.5" style={{ fontFamily: "'DM Sans',sans-serif" }}>
                      💡 Drag the blue pin to your exact door / gate if needed.
                    </p>
                  </div>
                )}
          ...
          <div className="slide-in" style={{ animationDelay: "120ms" }}>
            <SectionCard title="Date & Time" icon="📅">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide" style={{ fontFamily: "'DM Sans',sans-serif" }}>Select Date</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide" style={{ fontFamily: "'DM Sans',sans-serif" }}>Preferred Slot</label>
          ...
          <div className="slide-in" style={{ animationDelay: "240ms" }}>
            <SectionCard title="Price Summary" icon="💳" accent>
              <div className="space-y-2.5">
                {[["Base Price", `₹${service.price}`], ["Visiting Charge", `₹${VISITING_CHARGE}`]].map(([label, val]) => (
                ...
                <p className="text-xs text-gray-400 mt-1" style={{ fontFamily: "'DM Sans',sans-serif" }}>💡 Pay after service. No hidden charges.</p>
              </div>
            </SectionCard>
          </div>
